...
die "usage: pskulkarni_N-rawread-remover.pl <inputfq> <outputfq>" if @ARGV ne 2;  #if not two ARGV list then print usage

my ($input, $output) = @ARGV;

open (my $in, "<", $input) or die "error reading ARGV[0] for opening. Usage: program.pl <fastq file>";
...
#while loop: read file, only put those reads into a hash that have no N nucleotides in sequence
while (<$in>){ # while reading file
	$line = $_;            #set line equal to the currrent line
	$line_position++;      #set a line_position for each line (1-4, then restarts after 4)
	
	if ($line_position == 1){    #if line_position is 1 then set that to sequence name
		$seqname_line = $line;
	}
	
	elsif ($line_position == 2){     #if line pos is 2 then set that to sequence
		$seq_line = $line;
	}
	
	elsif ($line_position == 3){     #if line pos is 3 then set that to just 'third line'
		$third_line = $line;
	}
	
	elsif ($line_position == 4){     #if line pos is 4 then set that to quality line
		$line_position = 0;			 #reset positions
		$quality_line = $line;
		
		if ($seq_line !~ m/N/){      #if sequence doesn't have a N nucleotide then add all 4 read lines into hash
		$fastqhash{$seqname_line}{$seq_line}{$third_line}= $quality_line; 
		}
		elsif ($seq_line =~ m/N/){	 #if sequence has N then don't add into hash and add 1 to the count of rejected reads
			$rejected_count_N++;
		}
	}
